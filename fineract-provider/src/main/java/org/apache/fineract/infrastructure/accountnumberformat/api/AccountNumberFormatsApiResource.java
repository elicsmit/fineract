...
public class AccountNumberFormatsApiResource {
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Update an Account number format", tags = {"Account number format"})
    ...
    @Path("{accountNumberFormatId}")
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Delete an Account number format", tags = {"Account number format"}, description = "Note: Account numbers created while this format was active would remain unchanged.")
